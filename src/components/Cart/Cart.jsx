...
<h1  className='mb:font-semibold text-black'>total number of items: <span className='text-[#22DB14]'> {countItems}</span></h1>
    ...
    {cartdetails?.products?.map((product)=> <tr key={product.product._id} className="     flex flex-wrap   md:flex md:justify-between md:items-center   bg-gray-50 rounded-lg border-b dark:bg-gray-800 dark:border-gray-700  dark:hover:bg-gray-600">
        <td className="p-4 md:flex md:flex-nowrap md:items-center">
          <img src={product.product.imageCover} className="sm:w-full md:w-[30%] h-[200px] " alt="cart image"  />
          <td className="px-6 py-4 font-semibold text-black dark:text-white ">
          <div className=''>
          <p className='s md:text-lg font-semibold'>{product.product.title}</p>
